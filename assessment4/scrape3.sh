...
do
    ...
    #don't create directories from URLs (-nd) and instead create directories based on prefix (-P). There is no hsts database so disable that option (--no-hosts)
    wget -nd --no-hsts -P ./store/html/$publicationNames "$publicationURLs" 
    ...
    wget -nd --no-hsts -e -robots=off -r -A.pdf -P ./store/pdfs "$publicationURLs" 
    wget -nd --no-hsts -e -robots=off -r -A.docx,doc -P ./store/docs "$publicationURLs" 
    ...
    do
         ...
         #don't create directories from URLs (-nd) and instead create directories based on a prefix (-P). There is no hsts database so disable that option (--no-hosts)
         wget -nd --no-hsts -P ./store/html/$publicationNames/related $relatedDocsURLs
         #convert html files to text (html2text), remove some offending characters (-ascii), filter to remove the meaningless top and bottom sections and store in an appropriate folder
